...
void CHiveAgentComponent::Initialize()
{
	...
	m_pRewindComponent = m_pEntity->GetOrCreateComponent<CRewindComponent>();
	m_pRewindComponent->SetBaseFrame(CRewindFrame_HiveAgent::NewFrame());
...
void CHiveAgentComponent::ProcessEvent(SEntityEvent& event)
{
	...
	{
		REWINDSTATECHECK(m_pRewindComponent, CAPTURING)
...
void CHiveAgentComponent::Explore()
{
...
void CHiveAgentComponent::GoHome()
{
...
void CHiveAgentComponent::UpdateAI()
{
...
void CHiveAgentComponent::ForceAIImportantUpdate()
{
	m_AIUpdatesSinceLastImportant = AIUPDATE_IMPORTANT_FILTER;
}

void CHiveAgentComponent::UpdateMovement(float FrameTime)
{
	if (!m_bIsMoving)
		return;
...
void CHiveAgentComponent::SensePheromones()
{
...
CPheromoneComponent * CHiveAgentComponent::DropPheromone(int PheromoneType)
{
...
CPheromoneComponent * CHiveAgentComponent::GetStrongestPheromone(int PheromoneType)
{
...
std::vector<CPheromoneComponent*> CHiveAgentComponent::GetPheromones(int PheromoneType)
{
...
void CHiveAgentComponent::RemovePheromoneFromSensed(CPheromoneComponent * pPheromone)
{
	if (!pPheromone)
		return;
...
void CHiveAgentComponent::RemovePheromonesFromSensed(std::vector<CPheromoneComponent*> pPheromones)
{
...
float CHiveAgentComponent::GetDistanceFromPheromone(CPheromoneComponent * pPheromone)
{
	Vec3 vDist = m_pEntity->GetPos() - pPheromone->GetEntity()->GetPos();
	return vDist.GetLength();
}

void CHiveAgentComponent::EndBehavior()
{
	switch (m_CurrentBehavior)
	{
	case NONE:
...
void CHiveAgentComponent::Explore_Behavior()
{
	if (m_bBehaviorToEnd) {
...
void CHiveAgentComponent::Explore_End()
{
	GoHome();
}

void CHiveAgentComponent::GoHome_Behavior()
{
	if (m_bBehaviorToEnd) {
...
void CHiveAgentComponent::GoHome_End()
{
...
void CHiveAgentComponent::SetNewPheromoneDestination(CPheromoneComponent * pPheromone)
{
